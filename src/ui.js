  ...
  try {
    const response = await fetch(
      `https://api.pexels.com/v1/search?query=${encodeURIComponent(query)}&per_page=1&orientation=landscape`,
      {
        headers: {
          Authorization:
            "RBhECvORcQjs8GUlW3FXmy7WwczHde6Tbhke2hr2tWRyY4acANLRq6A4",
        },
      },
    );
...
async function renderWeather(weatherData, currentUnit) {
  ...
  for (let i = currentHour; i < hours.length && count < 6; i += 3) {
    ...
    const tempValue =
      currentUnit === "metric" ? hour.temperature.c : hour.temperature.f;
  ...
  weatherData.upcomingdays.forEach((day) => {
    ...
    const maxTempValue =
      currentUnit === "metric" ? day.tempmax.c : day.tempmax.f;
    const minTempValue =
      currentUnit === "metric" ? day.tempmin.c : day.tempmin.f;
